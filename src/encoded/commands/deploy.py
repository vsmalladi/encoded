...
def _get_cloud_config_yaml(main_args):
    ...
    if main_args.save_config_name:
        ...
        write_file_path = "{}/{}/{}-template.yml".format(
            main_args.conf_dir,
            'built-ymls',
            config_name,
        )
...
def _write_config_to_file(build_config, build_path, template_name):
    ...
    es_ip_arg = '--es-ip $ES_HEAD_IP' if template_name in ['app-pg', 'app'] else ''
    if es_ip_arg:
        deploy_cmd += ' ' + es_ip_arg
    pg_ip_arg = '--pg-ip $PG_DB_IP' if template_name in ['app-es', 'app'] else ''
    if pg_ip_arg:
        deploy_cmd += ' ' + pg_ip_arg
...
def main():
    ...
    if not assembled_template or not template_name:
        ...
        sys.exit(1)
    if save_path:
        _write_config_to_file(build_config, build_path, template_name)
        sys.exit(0)
    ...
    if main_args.dry_run:
        ...
        print('instances_tag_data', instances_tag_data)
        print('is_tag:', is_tag, ', is_branch:', is_branch)
        print('Dry Run')
        sys.exit(0)
    # AWS - Below
